...
import {loadEosProjectManifest} from './load';

const projectsDir = path.join(__dirname, '../../test');

describe('project.yaml', () => {
  it('can parse project.yaml to ProjectManifestImpl', () => {
    expect(loadEosProjectManifest(path.join(projectsDir, 'project.yaml'))).toBeTruthy();
  });

  it('can validate project.yaml', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_falsy.yaml'))).toThrow();
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_falsy_array.yaml'))).toThrow();
  });

  it('can validate a v0.2.0 project.yaml', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_0.2.0.yaml'))).not.toThrow();
  });

  it('can fail validation if version not supported', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_invalid_version.yaml'))).toThrow();
  });

  it('can validate a v0.2.0 project.yaml with a custom data source', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_0.2.0_custom_ds.yaml'))).not.toThrow();
  });

  it('can validate a v0.2.1 project.yaml with templates', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_0.2.1.yaml'))).not.toThrow();
  });

  it('can validate a v1.0.0 project.yaml with templates', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0.yaml'))).not.toThrow();
  });

  it('can convert genesis hash in v1.0.0 to chainId in deployment', () => {
    const deployment = loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0.yaml')).asV1_0_0.deployment;
    expect(deployment.network.chainId).not.toBeNull();
    console.log(deployment.network.chainId);
  });

  it('can get chainId for deployment', () => {
    const deployment = loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0_chainId.yaml')).asV1_0_0.deployment;
    expect(deployment.network.chainId).toBe('moonbeamChainId');
  });

  it('can get runner options for deployment', () => {
    const deployment = loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0_node_options.yaml')).asV1_0_0
      .deployment;
  ...
  it('can validate bypass blocks', () => {
    const deployment = loadEosProjectManifest(path.join(projectsDir, 'project_bypass.yaml')).asV1_0_0.deployment;
    const range_deployment = loadEosProjectManifest(path.join(projectsDir, 'project_bypass_range.yaml')).asV1_0_0
      .deployment;

    expect(deployment.network.bypassBlocks).not.toBeNull();
    expect(range_deployment.network.bypassBlocks).not.toBeNull();

    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_bypass.yaml'))).not.toThrow();
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_bypass_range.yaml'))).not.toThrow();
  });

  it('can validate a v1.0.0 project.yaml with unsupported runner node', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0_bad_runner.yaml'))).toThrow();
  });

  it('can throw error with unsupported runner version', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0_bad_runner_version.yaml'))).toThrow();
  });

  it('can validate a v1.0.0 project.yaml runner and datasource mismatches', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0_runner_ds_mismatch.yaml'))).toThrow();
  });

  it('can fail validation if custom ds missing processor', () => {
    expect(() => loadEosProjectManifest(path.join(projectsDir, 'project_0.2.0_invalid_custom_ds.yaml'))).toThrow();
  });

  it('can convert project with assets to deployment', () => {
    const manifest = loadEosProjectManifest(path.join(projectsDir, 'project_1.0.0.yaml'));
